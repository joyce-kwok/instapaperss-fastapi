...
def fetch_items(url: str):
    ...
    for li in news_list.select("li"):
        ...
        title = li.select_one(".c-link-text")
        title_str = title.get_text(strip=True) if title else "No title"


        # date
        date_el = li.select_one(".c-date")  # span with date text like 2026/01/09 [page:1]
        ...
        cat_el = li.select_one(".c-tag")  # e.g. キャンペーン / デジタル etc. [page:1]
